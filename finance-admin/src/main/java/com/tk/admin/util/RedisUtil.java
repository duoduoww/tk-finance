...
public class RedisUtil {
    ...
    private static int MAX_ACTIVE = PropertiesFileUtil.getInstance("redis").getInt("redis.maxActive");
    // 控制一个pool最多有多少个状态为idle(空闲的)的jedis实例，默认值也是8。
    private static int MAX_IDLE = PropertiesFileUtil.getInstance("redis").getInt("redis.maxIdle");
    // 等待可用连接的最大时间，单位毫秒，默认值为-1，表示永不超时。如果超过等待时间，则直接抛出JedisConnectionException；
    private static int MAX_WAIT = PropertiesFileUtil.getInstance("redis").getInt("redis.maxWaitmillis");
    // 超时时间
     ...
     * @return value
     */
    public synchronized static String getHasValue(String key, String field) {
        Jedis jedis = getJedis();
        if (null == jedis) {
            return null;
        }
     ...
     * @param field
     */
    public synchronized static void removeHas(String key,String field) {
        try {
            Jedis jedis = getJedis();
